...
def get_dataloader(data, batch_size):
    ...
    dataloder = DataLoader(ds, batch_size=batch_size, shuffle=True)
...
def bert_tokenize_for_valid_examples(batch_orginal_sentences, batch_noisy_sentences):
    ...
    batch_orginal_sentences3 = batch_orginal_sentences
    batch_noisy_sentences3 = batch_noisy_sentences

    temp1 = []
    temp2 = []
    for item1,item2 in zip(batch_orginal_sentences,batch_noisy_sentences):
        if len(item1.replace(" ","").replace("\n","")) < 2:
            if len(batch_orginal_sentences[0].replace(" ","").replace("\n","")) < 2:
                temp1.append(batch_orginal_sentences[1])
                temp2.append(batch_noisy_sentences[1])
            else:
                temp1.append(batch_orginal_sentences[0])
                temp2.append(batch_noisy_sentences[0])
        else:
            temp1.append(item1)
            temp2.append(item2)
    
    batch_orginal_sentences = tuple(temp1)
    batch_noisy_sentences = tuple(temp2)
    ...
    if len(valid_idxs)>0:
        for tokens in batch_tokens:
            if len(tokens) < 1:
                print("******")
                print("batch_orginal_sentences3")
                print(batch_orginal_sentences3)
                print("batch_noisy_sentences3")
                print(batch_noisy_sentences3)
                print("_batch_orginal_sentences")
                print(_batch_orginal_sentences)
                print("_batch_noisy_sentences")
                print(_batch_noisy_sentences)
                print("valid_idxs")
                print(valid_idxs)
