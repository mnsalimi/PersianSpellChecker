...
def load_model(vocab):
    model = SubwordBert(3*len(vocab["chartoken2idx"]),vocab["token2idx"][ vocab["pad_token"] ],vocab["token_freq"])
...
def spell_checking_on_sents(model, vocab, device, normalizer, txt):
    ...
    sents = [s for s in sents if len(s.replace("\n","").strip()) > 0]
...
if __name__ == '__main__':
    ...
    # vocab_path = "/home/marefat/projects/PSC/Nevise/vocab.pkl"
    # vocab_path = "/home/sekhavat/projects/Nevise_Cleaned/sekhavat_vocab.pkl"
    vocab_path = "/home/sekhavat/projects/Nevise_Cleaned/all5_vocab.pkl"
    # model_checkpoint_path = "/home/marefat/projects/PSC/Nevise/model.pth.tar"
    model_checkpoint_path = "/home/sekhavat/projects/Nevise_Cleaned/sekhavat_trains/0_new.pth"
    model, vocab, device = load_pre_model(vocab_path=vocab_path, model_checkpoint_path=model_checkpoint_path)
    
    

    #path_to_test = "/home/marefat/projects/PSC/test_sentences.txt"
    #path_to_test = "/home/marefat/projects/PSC/test_short_sent_and_words.txt"
    #path_to_test = "/home/marefat/projects/PSC/Nevise/test_samples/salimi_test.txt"
    # path_to_test = "/home/marefat/projects/PSC/Nevise/DATA/nevise-news-451_wrongs.txt"
    path_to_test = "/home/sekhavat/projects/Nevise_Cleaned/DATA/nevise-news-451_wrongs.txt"
    
    
    with open(path_to_test) as h:
      test = h.readlines()
    ...
    for t in tqdm(test):
      print(t)
      tic = time.time()
      output = spell_checking_on_sents(model, vocab, device, normalizer, t)
      toc = time.time()
      
      print(toc - tic)
      time_holder.append(toc - tic)
      
      with open("/home/sekhavat/projects/Nevise_Cleaned/DATA/nevise-news-451_wrongs_PREDICTED.txt", "a+") as h:
        h.seek(0)
        h.writelines(f"===========================================================\n")
        h.writelines(f"{output[0][0][0]}\n")
        h.writelines(f"-----------------------------------------------------------\n")
        h.writelines(f"{output[0][0][1]}\n")
        h.writelines(f"===========================================================\n")
        h.writelines("\n")
